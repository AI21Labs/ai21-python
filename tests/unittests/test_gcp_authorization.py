...
def test__get_gcp_credentials__when_project_id_not_found__should_raise_error(
    ...
    with pytest.raises(ValueError):
        auth.get_gcp_credentials(project_id=None)


@patch("google.auth.default")
@patch.object(GCPAuthorization, GCPAuthorization._get_gcp_request.__name__)
def test__get_gcp_credentials__when_project_id_found__should_set_auth(
    mock_gcp_request: Mock,
    mock_default: Mock,
    mock_gcp_credentials: Mock,
):
    ...
    auth = GCPAuthorization()
    credentials, project_id = auth.get_gcp_credentials(project_id=_TEST_PROJECT_ID)
...
@patch("google.auth.default")
@patch.object(GCPAuthorization, GCPAuthorization._get_gcp_request.__name__)
def test__get_gcp_credentials__project_id_mismatch__should_raise_error(
    mock_gcp_request: Mock,
    mock_default: Mock,
    mock_gcp_credentials: Mock,
):
    ...
    with pytest.raises(ValueError, match="Mismatch between credentials project id and 'project_id'"):
        _, _ = auth.get_gcp_credentials(project_id=_TEST_PROJECT_ID)
...
@patch("google.auth.default")
def test__get_gcp_credentials__should_return_credentials_and_project_id(
    mock_default: Mock,
    mock_gcp_credentials: Mock,
):
    mock_default.return_value = (mock_gcp_credentials, _TEST_PROJECT_ID)
...
def test__get_gcp_credentials__with_no_credentials__should_raise_error():
    auth = GCPAuthorization()
    auth.get_gcp_credentials = Mock(side_effect=ValueError("Could not get credentials for GCP project"))
...
@patch("google.auth.default")
@patch.object(GCPAuthorization, GCPAuthorization._get_gcp_request.__name__)
def test__get_gcp_credentials__with_default_credentials_error__should_raise_custom_credentials_error(
    mock_gcp_request: Mock,
    mock_default: Mock,
):
