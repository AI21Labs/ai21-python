...
class BaseAWSHttpClient(ABC):
    ...
    def _prepare_auth_headers(
        ...
        *,
        ...
        signer = SigV4Auth(credentials=credentials, service_name=service_name, region_name=session.region_name)
...
class AWSHttpClient(BaseAWSHttpClient):
    def __init__(
        ...
        if not url:
            url = f"https://bedrock-runtime.{self._aws_region}.amazonaws.com/model/{self._model_id}/invoke"
        self._url = url
    ...
    def execute_http_request(
        ...
        auth_headers = self._prepare_auth_headers(
            url=self._url, data=json.dumps(body), service_name="bedrock-runtime", method="POST"
        )
        try:
